...
import { Test7Answer, TEST7_PAGE1_QUESTIONS, TEST7_PAGE2_QUESTIONS, TEST7_PAGE3_QUESTIONS } from '../../types/test7Types';
...
const Test7Page3: React.FC = () => {
  const navigate = useNavigate();
  const dispatch = useAppDispatch();
  const isMobile = useIsMobile();
  const { page1, page2, page3, startedAt } = useAppSelector((state) => state.test7);
  const profile = useAppSelector((state) => state.profile);
  
  const [answers, setAnswers] = useState<{ [key: number]: Test7Answer }>(page3.answers);
  ...
  const handleAnswerChange = (questionId: number, answer: Test7Answer) => {
    ...
    // Сохраняем в Redux
    dispatch(setAnswer({ page: 'page3', questionId, answer }));
  ...
  const validateAnswers = (): boolean => {
    const unansweredQuestions = TEST7_PAGE3_QUESTIONS.filter(
  ...
  return (
    <TaskContainer>
      ...
      <QuestionsContainer>
        {TEST7_PAGE3_QUESTIONS.map((question) => {
